...
{
    public partial class DashboardPetani : UserControl
    {
        ...
        private void LoadPenanamanData()
        {
            try
            {
                List<PenanamanModel> penanamanList = PenanamanController.GetAllActivePenanaman();
                dgPenanaman.Grid.DataSource = penanamanList;
            }
            catch (Exception ex)
            {
                MessageBox.Show(
                    $"Error memuat data: {ex.Message}",
                    "Error",
                ...
                )
                {
                    ...
                    if (penanaman is not null)
                    {
                        ...
                        if (parentControl != null)
                        {
                            ...
                            // Subscribe to the KembaliClicked event
                            detailView.KembaliClicked += (s, ev) =>
                            {
                                parentControl.Controls.Remove(detailView); // Hapus detail view
                                detailView.Dispose(); // Dispose detail view
                                this.Show(); // Tampilkan kembali index view
                                this.BringToFront(); // Bawa index view ke depan
                            };

                            parentControl.Controls.Add(detailView);
                            detailView.BringToFront(); // Pastikan view baru di atas
                            this.Hide(); // Sembunyikan view index saat ini
                        }
                        else
                        {
                            MessageBox.Show(
